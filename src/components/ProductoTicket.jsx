...
const ProductoTicket = ({ entrada }) => {
  ...
  return (
    <div className="grid grid-cols-5 p-2 my-5 border rounded-lg">
      ...
      <div className="col-span-3 ml-2 text-sm">
        <p className="my-2"> {entrada.ticket.nombre}</p>
        <p className="my-2">Horario: {entrada.horario.horario}</p>
        <p className="my-2">Cantidad: {entrada.horario.asientos_seleccionados.length} </p>
        <p className="my-2">Sala: {entrada.ticket.sala}</p>
        <p>Asientos seleccionados: {entrada.horario.asientos_seleccionados.join(", ")}</p>
        <p>
          Precio: ${entrada.ticket.precio * entrada.horario.asientos_seleccionados.length}
        </p>
      </div>
